...
int original_plus_MOR_sending_X_MATRIX(vector<vector<double> >,double,double,double,string,int,string,vector<vector<double> >,vector<vector<double> >, vector<vector<double> >,vector<complex<double> > );
...
string C_matrix="./result/MPI_C_"+s_mynode+".txt";
string G_matrix="./result/MP_G_"+s_mynode+".txt";
...
//		MPI_Barrier(MPI_COMM_WORLD);
	...
	/*		cout<<"generating the reduced Matrices of orginal matrices"<<endl;
			...
			X_str_col=ss1.str();
			string out_file2="MOR_test_s"+s_mynode+"("+X_str_col+").txt";
...
*/	
string s1="./result/MPI_"+s_mynode+"_X_MATRIX.txt";
file_write(s1,X_MATRIX);
cout<<"done writing the X_MATRIX in txt file"<<endl;
...
vector<vector<double> > X_MATRIX1;
...
file_write("./result/MPI_X_BIG1.txt",X_big);
cout<<"done writing the first big"<<endl;
}
...
if(mynode==0)
{
file_write("./result/MPI_X_BIG2.txt",X_big);
cout<<"done writing second big"<<endl;
...
file_write("./result/MPI_X_SVD.txt",X_big);
	...
	vector<vector<double> >temp_X_MATRIX;
	retrun_X_MATRIX(temp_X_MATRIX,X_svd,count_a);
	X_MATRIX1=temp_X_MATRIX;  //saving for 0 th node
file_write("./result/1_X_MATRIX1.txt",X_MATRIX1);	


cout<<"MYNODE :"<<mynode<<" the number of rows in SVD X_MATRIX are : "<<temp_X_MATRIX.size()<<", the number of columns in SVD X_MATRIX are : "<<(temp_X_MATRIX.at(0)).size()<<endl;

	//converting temp_X_MATRIX vector into array for sending
	int row_X1=temp_X_MATRIX.size();
	int column_X1=(temp_X_MATRIX.at(0)).size();
	double X1_MATRIX_array[row_X1][column_X1];
	cout<<"Converting temp_X_MATRIX vector into array"<<endl;
	for(int i=0;i<temp_X_MATRIX.size();++i)
	{
		for(int j=0;j<(temp_X_MATRIX.at(0)).size();++j)
		{
			X1_MATRIX_array[i][j]=temp_X_MATRIX[i][j];
		}
	}
	cout<<"done converting X_svd into array"<<endl;


	cout<<"sending X1_MATRIX array from 0  node......................"<<endl;
	for(int i=1;i<numbernodes;++i)
	{

		MPI_Send(&row_X1,1,MPI_INT,i,1,MPI_COMM_WORLD);
		MPI_Send(&column_X1,1,MPI_INT,i,1,MPI_COMM_WORLD);
		MPI_Send(&X1_MATRIX_array,row_X1*column_X1,MPI_DOUBLE,i,1,MPI_COMM_WORLD);
	}

	cout<<"done sending X1_MATRIX from 0 node to other nodes..............................."<<endl;
}

else if(mynode!=0)
{
	int row_X1,column_X1;
	MPI_Recv(&row_X1,1,MPI_INT,0,1,MPI_COMM_WORLD,&status);
	MPI_Recv(&column_X1,1,MPI_INT,0,1,MPI_COMM_WORLD,&status);
	double X1_MATRIX_array[row_X1][column_X1];
	MPI_Recv(&X1_MATRIX_array,row_X1*column_X1,MPI_DOUBLE,0,1,MPI_COMM_WORLD,&status);
	// conveting array into vector

	vector<double> temp_X;
	for(int i=0;i<row_X1;++i)
	{
		for(int j=0;j<column_X1;++j)
		{
			temp_X.push_back(X1_MATRIX_array[i][j]);
		}
		X_MATRIX1.push_back(temp_X);
		temp_X.clear();
	}
string fn=s_mynode+"_X_MATRIX1.txt";
file_write(fn,X_MATRIX1);

}
MPI_Barrier(MPI_COMM_WORLD);

original_plus_MOR_sending_X_MATRIX(X_MATRIX1,f_max,f_min,points,filename,mynode,s_mynode,G_MATRIX,C_MATRIX,B_MATRIX,u_MATRIX);
//X_MATRIX1.clear();
MPI_Barrier(MPI_COMM_WORLD);
...
int original_plus_MOR_sending_X_MATRIX(vector<vector<double> > X_MATRIX,double frequency_max1,double frequency_min1,double points1,string filename,int mynode,string s_mynode,vector<vector<double> > G_MATRIX, vector<vector<double> > C_MATRIX, vector<vector<double> > B_MATRIX, vector<complex<double> > u_MATRIX)
{	
	double interval;
	...
	in_1=filename;
	/*
	   cout<<"MYNODE: "<<mynode<<"1) MAIN NETLIST FILE : "<<in_1<<endl;
	   vector<complex<double> > u_MATRIX;
	   vector<vector<double> >G_MATRIX;
	   vector<vector<double> >C_MATRIX;
	   vector<vector<double> >B_MATRIX;
...
ss1<<(X_MATRIX.at(0)).size();
X_str_col=ss1.str();
string out_SVD_file="SVD_MOR_test_s"+s_mynode+"("+X_str_col+").txt";
